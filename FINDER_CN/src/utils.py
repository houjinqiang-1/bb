...
class Utils:
    ...
    #计算给定解决方案solution在图graph上的鲁棒性。通过逐步添加节点，计算每个步骤下最大连通分量的大小，并最终计算总体鲁棒性。
    def getRobustness(self, graph, solution):
        ...
        graphutil = GraphUtil()
        disjoint_Set = DisjointSet(graph.num_nodes) #用于处理不相交集合的操作
        ...
        norm = (graph.num_nodes * (graph.num_nodes - 1)) / 2.0  #图的边数的一半，作为归一化的因子。
        for Node in reversed(solution):
            graphutil.recover_add_node(
                backupCompletedAdjListGraph, backupAllVex, currentAdjList, Node, disjoint_Set
            )
            totalMaxNum += disjoint_Set.ccd_score / norm
            self.MaxWccSzList.append(disjoint_Set.ccd_score / norm)
            temp = disjoint_Set.ccd_score / norm
        totalMaxNum = totalMaxNum - temp
        self.MaxWccSzList.reverse()
        return totalMaxNum / graph.num_nodes
