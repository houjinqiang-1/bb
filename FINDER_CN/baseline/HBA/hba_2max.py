...
def delnode(G1,G2, remove_nodes):
    closeness1 = nx.betweenness_centrality(G1)
    closeness2 = nx.betweenness_centrality(G2)

    max_cs = []
    for node in closeness1.keys():
        c1 = closeness1[node]
        c2 = closeness2[node]
...
def one_pass(G1,G2,N,M):
    ...
    for node in closeness1.keys():
        ...
        c2 = closeness2[node]
        max_cs.append((node, max(c1,c2)))
    sorted_max_cs = sorted(max_cs, key=lambda x: x[1], reverse=True)
    lastm = M
    remove_nodes = []
    i = 0
    while lastm != 1:
        remove_node = sorted_max_cs[i][0]
...
if __name__ == "__main__":
    datapath = '../../../data/'
    dataname = 'celegans_connectome_multiplex'
    #N = node_num[dataname]
    N = 279
    ...
    G1 = graphs[0]
    G2 = graphs[2]
    # adj1 = np.load("../adj1.npy")
    # adj2 = np.load("../adj2.npy")
    # N = len(adj1)
    # G1 = nx.Graph()
    # G2 = nx.Graph()
    ...
    for i in tqdm(range(n)):
        ...
        t1 = time.time()
        dN, MCCs, ps, remove_nodes = one_pass(G1, G2, N, M_ori)
        t2 = time.time()
        temp_score = (sum(MCCs)-1)/N + (N-len(remove_nodes)-1) / (M_ori * N)
        score += temp_score
    avg_score = score/n
    print('HBA score:',avg_score)
    print("time",t2-t1)
    file_path1 = "../../../results/HBA/none_cost/"+ "remove_nodes" + dataname + "_test.txt"
    with open(file_path1, 'w') as file:
        for c in remove_nodes:
            file.write(str(c) + '\n')
    file_path2 = "../../../results/HBA/none_cost/MaxCCList_Strategy_"+ dataname + "_test.txt"
