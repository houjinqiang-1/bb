...
class SupervisedGraphSage(nn.Module):
    ...
    def forward(self, nodes,is_test=False):
       ...
       # celegans
       #  if self.predict_layer == 0:
       #      result = embeds1 + 0.4992 * embeds2 + 0.5008 * embeds3
       #  elif self.predict_layer == 1:
       #      result = embeds1 + 0.5084 * embeds2 + 0.4916 * embeds3
       #  elif self.predict_layer == 2:
       #      result = embeds1 + 0.4964 * embeds2 + 0.5036 * embeds3
       #  else:
       #      print('has error: {}'.format(self.predict_layer))
    ...
    def get_data(self,nodes):
        embeds,predict = self.forward(nodes)
...
#                      torch.save(graphsage[k_i].state_dict(), './new-ablation/celegans/bitwise/_{}_{}.pkl'.format(best_epoch,k_i))
#                  best_test = test_Auc
#                  patience = 0
#             else:
#                  patience = patience + 1
...
#                  break
#             files = glob.glob('./new-ablation/celegans/bitwise/*.pkl')
#             for file in files:
...
#             os.remove(file)
    for k_j in range(lay_num):
        graphsage[k_j].load_state_dict(torch.load('./new-ablation/ckm/bitwise/_{}_{}.pkl'.format(101,k_j)))#best_epoch
    ...
    with open("./new-ablation/ckm/bitwise/ckm-second-bitwise.pkl", "wb") as f:
