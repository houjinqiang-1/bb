...
id_layer = 'PAP'   # 要预测第几层 'PTP', 'PLP', 'PAP',
...
def get_data_test():
    ...
    adj_ = adjs[id_layer]
    temp_adj = adj_ + np.eye(3025)
    temp_origin_adj  = sp.csr_matrix(temp_adj)
    ...
    while (cnt < len(contents) - 1):
        ...
        cnt += 1
    idx_val = (train_idx[int(np.floor(len(train_idx) * 0.9)):])

    # origin_adj = np.ones((3025,3025))
    # for i in range(2900):
    #     for j in range(2900):
    #         origin_adj[i,j] = 0

    origin_adj = temp_origin_adj.todense()
    #return features, origin_adj, break_adj, train_idx,test_idx
    print(origin_adj.shape, break_adj.shape, features.shape, len(train_idx), len(idx_val), len(test_idx))
    print(origin_adj.dtype, break_adj.dtype, features.dtype)
    print(origin_adj, break_adj, features, train_idx, idx_val, test_idx)
    return  features, origin_adj, break_adj, train_idx, idx_val, test_idx
