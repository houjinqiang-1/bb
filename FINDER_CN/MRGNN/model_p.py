...
from utils import *
from sklearn.metrics import roc_auc_score, f1_score,average_precision_score,f1_score
...
class SupervisedGraphSage(nn.Module):
    ...
    def get_data(self,nodes):
        embeds,predict,weight = self.forward(nodes,is_test=True)
        return embeds,predict,weight
...
def run(emb_dim, times, dataname="ckm", num_samples1=5, num_samples2=5, batch_num=256, break_por=0.1,epoches=2):
    np.random.seed(1)
    random.seed(1)
    if dataname == "ckm":
        ori_adj, adj, feat_data, idx_train, idx_train1, idx_train2, idx_val, idx_val1, idx_val2, idx_test, idx_test1, idx_test2 = [
            np.array(item) for item in load_data_ckm_mutilayer(break_por=break_por)]
    if dataname == "acm":
    ...
    agg_two.append(MeanAggregator(lambda nodes: enc_one[2](nodes).t(), cuda=True))
    ''''''
...
#                    torch.save(graphsage[k_i].state_dict(), './mode_pkl/new-mpk/bitwise/amazon/_{}_{}.pkl'.format(best_epoch,k_i))
#                best_test = test_Auc
#                patience = 0
#            else:
#                patience = patience + 1
...
#                break
#            files = glob.glob('./mode_pkl/new-mpk/bitwise/amazon/*.pkl')
#            for file in files:
...
#        graphsage[k_j].load_state_dict(torch.load('./mode_pkl/new-mpk/ckm/_{}_{}.pkl'.format(65,k_j)))#best_epoch

#    test_all = []
#    all_layer_embeds=[];all_layer_predict=[];all_layer_weight=[];
#
#    test_loss = []
#    test_acc = []
#    test_Auc = []
...
#        num_batch = len(test_data_pair)
#        layer_embeds=[];layer_predict=[];layer_weight=[];
#        for test_batch in test_data_pair:
#            test_node, test_targets = test_batch
#            batch_embeds,batch_predict,batch_weight = graphsage[kv].get_data(test_node)
#            layer_embeds.append(batch_embeds)
#            layer_predict.append(batch_predict)
#            layer_weight.append(batch_weight)
#            test_loss_t = graphsage[kv].loss(test_node, test_targets)
#            test_acc_t = graphsage[kv].acc(test_node, test_targets)
#            test_Auc_t = graphsage[kv].Auc(test_node, test_targets)
#            test_loss_p.append(test_loss_t)
...
#        all_layer_weight.append(layer_weight)
#    test_loss = (sum(test_loss)) / lay_num
#    test_acc = (sum(test_acc)) / lay_num
#    test_Auc = (sum(test_Auc)) / lay_num
#    print("test loss: {:.4f}  test acc: {:.4f}  test Auc: {:.4f}".format(test_loss, test_acc, test_Auc))
...
#    data_test['all_layer_weight'] = all_layer_weight
#    with open("./mode_pkl/new-mpk/ckm_test.pkl", "wb") as f:
...
#    graphs = ['amazon']
    ...
    for cnt, dataname in enumerate(graphs):
        ...
        emb_dim = [128]#[2,4,8,16,32,64,128,256,512,1024,2048]
#        p_nums = [2,4,8,16,32,64,128,256,512,1024,2048]
        for dim in emb_dim:
            print('embedding dim {}'.format(dim))
#            print('aggregate neigh num {}'.format(p_num))
