...
class FINDER:
    ...
    def Evaluate(self, data_test, data_test_name, dirt, model_file=None):
        ...
        for i in tqdm(range(n_test)):
            adj1 = np.load(f"./synthetic/{dirt}/syn_%s/adj1_%s.npy" % (data_test_name, i))
            adj2 = np.load(f"./synthetic/{dirt}/syn_%s/adj2_%s.npy" % (data_test_name, i))
    ...
    def EvaluateRealData(self, model_file, data_test, save_dir, stepRatio, num_nodes, layers):  # 测试真实数据
        ...
        save_dir_local, "MaxCCList_Strategy", test_name.split('.')[0], layers[0], layers[1], 'txt')
        ...
        cost.append(score)  # 把score加到记录cost的文件里
