...
#                      torch.save(graphsage[k_i].state_dict(), './ablation/lazega/semantic/_{}_{}.pkl'.format(best_epoch,k_i))
#                  best_test = test_Auc
#                  patience = 0
#             else:
#                  patience = patience + 1
...
#                  break
#             files = glob.glob('./ablation/lazega/semantic/*.pkl')
#             for file in files:
...
#             os.remove(file)
    for k_j in range(lay_num):
        graphsage[k_j].load_state_dict(torch.load('./ablation/lazega/semantic/_{}_{}.pkl'.format(349,k_j)))#best_epoch
    ...
    for kv in range(lay_num):
        ...
        test_Auc_p = []
        test_data_pair = sample_test_batch(idx_test[kv], ori_labels[kv], ori_adj[kv])
        test_all.append([nn for nn in range(71)])
        num_batch = len(test_data_pair)
        layer_embeds=[];layer_predict=[];layer_weight=[];
        for test_batch in test_data_pair:
            test_node, test_targets = test_batch
            batch_embeds,batch_weight = graphsage[kv].get_data([nn for nn in range(71)])
    ...
    with open("./ablation/lazega/lazega-semantic.pkl", "wb") as f:
