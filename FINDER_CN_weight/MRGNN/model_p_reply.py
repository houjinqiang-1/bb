...
#        writer.add_scalars('auc', {'auc': sum(epoche_auc) / len(epoche_auc), }, epoch)
        ...
        if (epoch+1)%2 == 0:
            ...
            if test_Auc > best_test:
                 ...
                 for k_i in range(lay_num):
                     torch.save(graphsage[k_i].state_dict(), './mode_pkl/attention/bitwise/celegans/_{}_{}.pkl'.format(best_epoch,k_i))
                 best_test = test_Auc
                 patience = 0
            else:
                 patience = patience + 1
            if patience > 5 & epoch > 100:
                 break
            files = glob.glob('./mode_pkl/attention/bitwise/celegans/*.pkl')
            for file in files:
    ...
    for k_j in range(lay_num):
        graphsage[k_j].load_state_dict(torch.load('./mode_pkl/attention/bitwise/celegans/_{}_{}.pkl'.format(best_epoch,k_j)))#best_epoch
    ...
    with open("./mode_pkl/attention/bitwise/celegans/celegans_test.pkl", "wb") as f:
