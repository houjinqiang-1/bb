...
#            print('has error: {}'.format(self.predict_layer))
        ...
        # bitwise
        # if self.predict_layer == 0:
        #     result = embeds1 + 0.4983*embeds2 + 0.5017*embeds3
        # elif self.predict_layer == 1:
        #     result = embeds1 + 0.4933*embeds2 + 0.5067*embeds3
        # elif self.predict_layer == 2:
        #     result = embeds1 + 0.5006*embeds2 + 0.4994*embeds3
        # else:
        #     print('has error: {}'.format(self.predict_layer))
    ...
    def get_data(self,nodes):
        embeds,weight = self.forward(nodes,is_test=True)
...
#        writer.add_scalars('auc', {'auc': sum(epoche_auc) / len(epoche_auc), }, epoch)
        ...
        if (epoch+1)%2 == 0:
            ...
            if test_Auc > best_test:
                 ...
                 for k_i in range(lay_num):
                     torch.save(graphsage[k_i].state_dict(), './ablation/celegans/semantic/_{}_{}.pkl'.format(best_epoch,k_i))
                 best_test = test_Auc
                 patience = 0
            else:
                 patience = patience + 1
            if patience > 5 & epoch > 100:
                 break
            files = glob.glob('./ablation/celegans/semantic/*.pkl')
            for file in files:
    ...
    for k_j in range(lay_num):
        graphsage[k_j].load_state_dict(torch.load('./ablation/celegans/semantic/_{}_{}.pkl'.format(best_epoch,k_j)))#best_epoch
    ...
    for kv in range(lay_num):
        ...
        test_Auc_p = []
        test_data_pair = sample_test_batch(idx_test[kv], ori_labels[kv], ori_adj[kv])
        test_all.append([nn for nn in range(279)])
        num_batch = len(test_data_pair)
        layer_embeds=[];layer_predict=[];layer_weight=[];
        for test_batch in test_data_pair:
            test_node, test_targets = test_batch
            batch_embeds,batch_weight = graphsage[kv].get_data([nn for nn in range(279)])
    ...
    with open("./ablation/celegans/fix-semantic/celegans-fix-semantic.pkl", "wb") as f:
